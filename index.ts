...
export default function contribute(plugin: PluginContext) {
  plugin.handle(scan, async (_input, { paseo }) => {
    const { config } = await paseo.config.get();
    const overrides = ((config as { agents?: { providers?: ProviderOverrides } }).agents?.providers ?? {}) as ProviderOverrides;
  ...
  plugin.handle(wireProvider, async ({ provider, email, dir }, { paseo }) => {
  ...
  plugin.handle(diagnoseProvider, async ({ providerId }, { paseo }) => {
    try {
  ...
  plugin.handle(mcpOverview, async () => {
